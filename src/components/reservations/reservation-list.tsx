    ...
    <UITable>
      <TableHeader>
        <TableRow>
          ...
          <TableHead className="text-center">الطاولة</TableHead>
      ...
      <TableBody>
        {reservations.length === 0 ? (
          ...
          reservations.map((res) => (
            <TableRow key={res.id}>
              <TableCell>
                {format(res.reservationTime, "p", { locale: ar })}
              </TableCell>
              <TableCell>
              ...
              <TableCell>
                {res.tables.length > 0
                  ? res.tables.map((t) => `طاولة ${t.number}`).join(", ")
                  : "-"}
              </TableCell>
...
function CheckInDialog({
  ...
  reservation: Reservation & { tables: Table[] };
  tables: Table[];
}) {
